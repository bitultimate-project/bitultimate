...
#if defined(NDEBUG)
# error "Bitultimate cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfcd02d3ca378b855e857bc4db29edf5f9cb4b0578b1467a7553c7b1487c4e648");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitultimate: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitultimate Signed Message:\n";
...
{
    // Bitultimate: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 7000 blocks
    nSubsidy >>= (nHeight / 7000); // Bitultimate: 7k blocks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 24 * 60 * 60; // Bitultimate: 10 days
static const int64 nTargetSpacing = 2 * 60; // Bitultimate: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitultimate: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitultimate: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9564eb53561097ef517328ed7451aca003147d8affb0ea8f0e0d5e05a3687f92");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1547464911;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085578177;

        if (fTestNet)
        {
            block.nTime    = 1547464887;
            block.nNonce   = 389498247;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
...
#if defined(_M_X64) || defined(_x86_64_) || defined(_M_AMD64) || (defined(MAC_OSX) && defined(_i386))
	...
	printf("block.GetHash = %s\n", block.GetHash().ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0x646296eb3ffe538bb84f3685b8f4f87323a02428573111629a6503e25af086b9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Bitultimate: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitultimateMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitultimateMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitultimateMiner(CWallet *pwallet)
{
    printf("BitultimateMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitultimate-miner");
    ...
    try { loop {
        ...
        printf("Running BitultimateMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitultimateMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitultimateMiner, pwallet));
}
